...
CV_mod <- lmerTest::lmer(CV ~ 0 + Study + Study:Diagnosis + (1 + Diagnosis|ID), data = scaled_data, control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE,  optCtrl = list(ftol_abs = 1e-10, xtol_abs = 1e-10, maxeval=10000)))
...
CV_mod2 <- lmerTest::lmer(CV ~ 0 + Diagnosis + (1 + Diagnosis|ID), data = scaled_data, control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE,  optCtrl = list(ftol_abs = 1e-10, xtol_abs = 1e-10, maxeval=10000)))
summary(CV_mod2) # An overall significant effect of Diagnosis on pitch variability... but a very small effect size. As the plot and the first model demonstrates, the effect size (and direction!) are both rather inconsistent.
...
### ASK SOMEONE ABOUT CONVERGENCE ISSUES NEXT TIME WE HAVE THE CHANCE!!!

prop_mod <- lmerTest::lmer(Speech_Prop ~ 0 + Study + Study:Diagnosis + (1 + Diagnosis|ID), data = scaled_data, control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE,  optCtrl = list(ftol_abs = 1e-10, xtol_abs = 1e-10, maxeval=10000)))
summary(prop_mod)  #No significant effects, and results go in rather different directions.

#Pooled together:
prop_mod2 <- lmerTest::lmer(Speech_Prop ~ 0 + Diagnosis + (1 + Diagnosis|ID), data = scaled_data, control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE,  optCtrl = list(ftol_abs = 1e-10, xtol_abs = 1e-10, maxeval=10000)))
summary(prop_mod2) # Non-significant.

## Speechrate

ggline(scaled_data, x = "Diagnosis", y = "Speechrate",col='Study', add = c("mean_se", "dodge"), palette = "jco")

sr_mod <- lmerTest::lmer(Speechrate ~ 0 + Study + Study:Diagnosis + (1 + Diagnosis|ID), data = scaled_data, control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE,  optCtrl = list(ftol_abs = 1e-10, xtol_abs = 1e-10, maxeval=10000)))
summary(sr_mod) #Three significant effects + many of the null results go in the expected direction.

sr_mod2 <- lmerTest::lmer(Speechrate ~ 0 + Diagnosis + (1 + Diagnosis|ID), data = scaled_data, control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE,  optCtrl = list(ftol_abs = 1e-10, xtol_abs = 1e-10, maxeval=10000)))
summary(sr_mod2)

## Pause Duration

# Since some of the participants have 0 pauses (somehow) on some of their trials, this creates mathematical problems when defining pause duration (division by 0). Thus, we remove all the rows where npause = 0.
no_zeros <- scaled_data %>% subset(npause != 0)

no_zeros$Diagnosis <- no_zeros$Diagnosis %>% as.factor()
no_zeros$Study <- no_zeros$Study %>% as.factor()

ggline(no_zeros, x = "Diagnosis", y = "P_Dur",col='Study', add = c("mean_se", "dodge"), palette = "jco")
